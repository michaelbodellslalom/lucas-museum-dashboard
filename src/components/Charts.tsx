...
export function MembershipChart({ channels, levels }: { channels: CategoryValue[]; levels: CategoryValue[] }) {
  ...
  return <div className="membership-chart">
    ...
    <AccessibleChart label="Memberships sold by level" height={250}>
      <ResponsiveContainer width="100%" height="100%"><BarChart data={levelData} layout="vertical" margin={{ top: 8, right: 20, bottom: 8, left: 8 }}>
        <CartesianGrid stroke="#e6e2da" horizontal={false} />
        <XAxis type="number" hide />
        <YAxis dataKey="name" type="category" tickLine={false} axisLine={false} width={74} fontSize={11} />
        <Tooltip contentStyle={tooltipStyle} />
        <Bar dataKey="value" name={view === 'selected' ? 'Selected period' : 'Prior comparable'} fill="#4f7b65" radius={[0, 3, 3, 0]}><LabelList dataKey="value" position="right" fontSize={11} /></Bar>
...
export function DemographicPriceChart({ data }: { data: DemographicValue[] }) {
  ...
  return <AccessibleChart label="Visitor volume and average ticket price by aggregated visitor demographic" height={280}>
    <ResponsiveContainer width="100%" height="100%"><ComposedChart data={chartData} margin={{ top: 18, right: 12, bottom: 4, left: -8 }}>
      <CartesianGrid stroke="#e6e2da" vertical={false} />
      <XAxis dataKey="name" tickFormatter={(value) => shortLabels[value] ?? value} tickLine={false} axisLine={false} fontSize={9} interval={0} />
      <YAxis yAxisId="visitors" tickLine={false} axisLine={false} fontSize={10} />
      <YAxis yAxisId="price" orientation="right" domain={[0, 40]} tickFormatter={(value) => `$${value}`} tickLine={false} axisLine={false} fontSize={10} />
...
export function FunnelChart({ data }: { data: CategoryValue[] }) {
  ...
  return <div className="funnel-chart" role="img" aria-label="Digital ticket purchase funnel from ticket page to membership add-on">
    {chartData.map((step, index) => {
      const width = Math.max(26, (step.value / max) * 100)
      const detail = index === 0 ? `${step.value.toLocaleString()} sessions` : `${step.value.toLocaleString()} sessions · ${lossShares[index - 1]}% of total drop-off`
...
export function CapacityChart({ data }: { data: TimePoint[] }) {
  ...
  return <AccessibleChart label="Capacity utilization by completed operating hour">
    <ResponsiveContainer width="100%" height="100%"><AreaChart data={chartData} margin={{ top: 12, right: 16, bottom: 4, left: -18 }}>
      <CartesianGrid stroke="#e6e2da" vertical={false} />
      <XAxis dataKey="label" tickLine={false} axisLine={false} fontSize={11} />
      <YAxis domain={[0, 100]} tickFormatter={(value) => `${value}%`} tickLine={false} axisLine={false} fontSize={11} />
      <Tooltip contentStyle={tooltipStyle} formatter={(value) => `${value}%`} />
      <ReferenceLine y={85} stroke="#bf6449" strokeDasharray="5 4" label={{ value: 'At risk 85%', fontSize: 10, fill: '#8d4230' }} />
      <ReferenceLine y={70} stroke="#927039" strokeDasharray="4 4" label={{ value: 'Watch 70%', fontSize: 10, fill: '#735a2e' }} />
...
export function HorizontalBarChart({ data, label, unit = '' }: { data: CategoryValue[]; label: string; unit?: string }) {
  ...
  return <AccessibleChart label={label} height={Math.max(240, data.length * 38)}>
    <ResponsiveContainer width="100%" height="100%"><BarChart data={chartData} layout="vertical" margin={{ top: 8, right: 40, bottom: 8, left: 22 }}>
      <CartesianGrid stroke="#e6e2da" horizontal={false} />
      <XAxis type="number" hide />
      <YAxis dataKey="name" type="category" tickLine={false} axisLine={false} width={92} fontSize={11} />
      <Tooltip contentStyle={tooltipStyle} formatter={(value) => `${Number(value).toLocaleString()}${unit}`} />
      <Bar dataKey="value" fill="#285f7a" radius={[0, 3, 3, 0]}><LabelList dataKey="value" position="right" formatter={(value) => `${Math.round(Number(value)).toLocaleString()}${unit}`} fontSize={11} /></Bar>
    </BarChart></ResponsiveContainer>
  </AccessibleChart>
}
