import { ArrowDownRight, ArrowRight, ArrowUpRight, Clock3, Gauge, Info, MapPin, Minus, UsersRound } from 'lucide-react'
import {
  CapacityChart,
  FlowMap,
  Heatmap,
...
function MiniMetric({ label, value, detail, definition, comparison, phaseTwo = false, icon: Icon }: { label: string; value: string; detail: string; definition?: string; comparison?: MetricComparison; phaseTwo?: boolean; icon: typeof Gauge }) {
  const TrendIcon = comparison?.direction === 'up' ? ArrowUpRight : comparison?.direction === 'down' ? ArrowDownRight : Minus
  return <article className={`mini-metric ${phaseTwo ? 'mini-metric-phase-two' : ''}`}><div className="mini-metric-heading"><Icon size={19} /><div className="mini-metric-actions">{definition && <button className="info-button" aria-label={`Definition: ${definition}`} data-tooltip={definition}><Info size={15} /></button>}</div></div><span>{label}</span><strong>{value}</strong>{comparison && <div className={`mini-metric-comparison trend trend-${comparison.tone}`}><TrendIcon size={15} />{comparison.label}{comparison.secondaryLabel && <><i>/</i><TrendIcon size={15} />{comparison.secondaryLabel}</>}</div>}<small>{detail}</small>{phaseTwo && <span className="phase-two-badge">Coming in Phase 2</span>}</article>
}
    ...
    <div className="page operations-page">
      ...
      <section aria-labelledby="glance-title">
        ...
        <div className="mini-metric-grid operating-picture-grid">
          ...
          <MiniMetric icon={Clock3} label="Avg Elevator Wait Time" value={`${averageElevatorWait} min`} detail={`vs. ${data.comparisonLabel}`} comparison={{ direction: elevatorWaitTrend >= 0 ? 'up' : 'down', label: `${Math.abs(elevatorWaitTrend)} min`, tone: elevatorWaitTrend > 0 ? 'bad' : 'good' }} definition="Average observed or estimated elevator wait during the selected completed period." phaseTwo />
          <MiniMetric icon={UsersRound} label="Avg Elevator Party Size" value={`${elevatorPartySize} people`} detail={`vs. ${data.comparisonLabel}`} comparison={{ direction: 'up', label: '1 person', tone: 'bad' }} definition="Average visitors per observed elevator party based on aggregated instrumentation." phaseTwo />
          <MiniMetric icon={Clock3} label="Avg Museum Visit Duration" value={formatDuration(museumVisitMinutes)} detail={`vs. ${data.comparisonLabel}`} comparison={{ direction: 'up', label: `${Math.round(11 * data.averageFactor)} min`, tone: 'good' }} definition="Average elapsed time between visitor arrival and departure during the selected period." phaseTwo />
