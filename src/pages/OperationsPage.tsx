...
function MiniMetric({ label, value, detail, definition, comparison, icon: Icon }: { label: string; value: string; detail: string; definition?: string; comparison?: MetricComparison; icon: typeof Gauge }) {
  ...
  return <article className="mini-metric"><div className="mini-metric-heading"><Icon size={19} /><div className="mini-metric-actions">{definition && <button className="info-button" aria-label={`Definition: ${definition}`} data-tooltip={definition}><Info size={15} /></button>}</div></div><span>{label}</span><strong>{value}</strong>{comparison && <div className={`mini-metric-comparison trend trend-${comparison.tone}`}><TrendIcon size={15} />{comparison.label}{comparison.secondaryLabel && <><i>/</i><TrendIcon size={15} />{comparison.secondaryLabel}</>}</div>}<small>{detail}</small></article>
}
